...
def build_validator_system(prior_notes: list[str]) -> str:
    ...
    return (
        ...
        "Be strict but fair. If a field is empty, vague ('no information "
        "available'), or unrelated to the user's question, mark insufficient. "
        "If the user asked about the CEO and the findings only mention "
        "products, mark insufficient with a note like 'add CEO / leadership "
        "context'. If confidence_score < 6 the findings are almost certainly "
        "insufficient unless the user asked something the data genuinely "
        "can't answer." + note_block
    )
...
def build_synthesis_system() -> str:
    return (
        ...
        "Rules:\n"
        "  - Lead with the most relevant fact for the question asked.\n"
